...
4. Give top-level sections `id`s and real `h2`s. On wide viewports a left sidebar table of contents builds itself from them (a `data-short` attribute on an `h2` shortens its label); pages with fewer than four such sections get no sidebar, which is correct for them. When one section grows to many screenfuls, mark its inner chunk headings with `data-toc="Short label"`; each becomes an indented sidebar entry with an automatic id, so the longest part of the page stays navigable.
