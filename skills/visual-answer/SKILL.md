...
One thing earns full width: content that **is** the argument rather than illustrating it. A row of captured screens being compared, a matrix whose whole point is seeing every cell at once, a diagram that only reads at scale. Those break out of the column with the full-bleed row in `references/patterns.md` and go as wide as they need; the prose around them stays in the column, so the page still reads as one document with a few things spanning it. A comparison forced to scroll horizontally, or shrunk to fit a measure meant for sentences, has been made unreadable to protect a rule about sentences.
