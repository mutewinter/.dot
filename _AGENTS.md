...
- Hang your recommendation off each item as a nested sub-bullet carrying nothing else, opening with a one-to-three-word verdict: `   - <glyph> <verdict>: <one-clause why>`. The glyph gives the answer's shape before I read it: 👉 take the action, 🛑 don't (skip it, keep what's there, leave it alone), 🤷 a genuine coin flip, ⏸️ I'm stopped until you answer, so that item has no default. Recommend even when the call is close; 🤷 only when you truly have no preference.
